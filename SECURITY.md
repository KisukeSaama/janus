...
| Authorization | A grant binds one application to one provider and one credential, and is decided before OpenBao is read. It admits the caller to the whole API rather than to a subset of its paths: the API's own authorization for the credential Janus presents is what limits reach, and a second copy of that answer here could only go stale. A grant may still narrow itself where that answer cannot be had — a path prefix and a set of methods, for the keys no upstream can scope — and may refuse the connected account's identity, so reaching a person's own data is a decision an operator takes rather than a header any caller can write. |
