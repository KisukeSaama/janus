...
import { useApplications, useAudit, useCredentials, useGrants, useProviders, type Audit } from '../../api';
import { Empty, LiveState, Outcome, PageHead, SectionHead, SkeletonRows } from '../../components';
...
/**
 * The console's home is the statement an operator maintains, not the records it is stored in:
 * this service may call that API on these routes, and it currently works.
 *
...
export function ConnectionsPage({
  ...
  onNavigate: (to: AttentionTarget | 'activity') => void;
}) {
  const { t } = useI18n();
  ...
  return (
    <>
      <PageHead
        ...
        intro={t('connections.lead')}
        action={connectButton}
      />

      {/* Held open at its own height while the records load, so the list below does not step down
          the page the moment the findings arrive. */}
      {loading ? (
        <div className="panel mb-8 flex items-start gap-3 px-4 py-3.5" aria-hidden="true">
        ...
        <NeedsAttention
          items={findings}
          onReview={(item) => (item.target === 'connections' ? setFocus(item.ids) : onNavigate(item.target))}
        />
      )}
      ...
      <section className="mt-10">
        ...
        {events.isPending ? (
          ...
          <p className="panel px-4 py-8 text-center text-sm text-text-2">{t('connections.noEvents')}</p>
        ) : (
          <ul className="panel divide-y divide-line">
...
function ConnectionRow({ connection, onOpen }: { connection: Connection; onOpen: () => void }) {
  ...
  return (
    <li>
      <button
        onClick={onOpen}
        className="grid w-full grid-cols-1 items-center gap-x-6 gap-y-3 px-4 py-3.5 text-left transition-colors duration-150 hover:bg-sunk lg:grid-cols-[minmax(0,1.05fr)_minmax(0,1fr)_auto]"
      >
...
function NeedsAttention({ items, onReview }: { items: Attention[]; onReview: (item: Attention) => void }) {
  ...
  if (items.length === 0) {
    return (
      <div className="panel mb-8 flex items-start gap-3 px-4 py-3.5">
