...
**It opens on a list of APIs, not on an empty form.** Registering Spotify asks for a destination, a strategy, a token endpoint and an authorisation page, and every one of those has a single correct answer published in somebody's documentation. A form that asks for them is asking the reader to leave, and they come back with a token endpoint pasted from a blog post. So the first question is the one they can answer without leaving — *which API is it* — and choosing from the list answers the rest. Everything it filled in stays editable, an API that is not in the list is one click away, and what a preset cannot do for you it says out loud: Twitch also wants a `Client-Id` header, Notion a version, Reddit a real user agent.
