...
Administrators can register an API and its authentication contract, then provision their own credential. Ordinary users enter through the catalogue: search, activate, and provide only the credential fields required by the administrator-selected strategy. The public path is global — `/gateway/{slug}` — because the application key and grant already identify who may call it.
