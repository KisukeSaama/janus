...
The project is a Java 25 / Spring Boot 4 modular monolith with a React administration console. Each module is layered controller → service → repository: the HTTP class decides nothing, the service owns the transaction and the audit record, and the entities enforce their own invariants. PostgreSQL contains identity, provider, grant, and audit metadata. It never contains plaintext external secrets.
...
- The administrator password is cached the same way and for the same reason. HTTP Basic is stateless, so every console request would otherwise repeat a comparison the hash was tuned to make expensive — roughly a tenth of a second each, several per screen, and a multiplier on anything sent in bulk. Only verified passwords are remembered, keyed on a digest of the value together with the hash it was checked against — which is what makes a changed password invalidate its own entries, since accounts live in the database and the new hash simply does not match a key built from the old one. Changing your own password requires the current one, and ends every other console session that account holds: a session left open must not be enough to replace the password that opened it, and somebody changing theirs because they believe it was learned is asking for exactly that. An administrator resetting somebody else's cannot supply the old one and is not asked to, and none of the target's sessions are spared.
...
- A client id is not a secret, and an API that wants one on the request itself says so on the destination rather than on every caller. Twitch refuses a Helix call without `Client-Id` whatever token it carries, so the catalogue entry names that header and Janus fills it with the left half of the stored `client_id:client_secret`, read from OpenBao at call time. A calling service that had to send it would be holding a piece of the authentication contract, and rotating the OAuth client would mean chasing every service that hard-coded half of it. Free-form constant headers were considered and left out: the first thing one would carry is an API key, in plaintext, in Postgres.
...
For local backend work without Docker, run PostgreSQL and OpenBao from the development Compose file, then use the Maven wrapper with a Java 25 toolchain:
...
| `JANUS_ALLOW_PRIVATE_DESTINATIONS` | `false` | **Disables SSRF address filtering,** loopback included. Only for pointing a development provider at a private host. |
| `JANUS_PRIVATE_DESTINATIONS_ENABLED` | `false` | Lets an API be registered as living on your local network, for a self-hosted service that exists nowhere else. Decided per API, never admits loopback or link-local, and hidden from the console while off. |
...
Both Compose files derive these from the host the deployment already declares, so a Compose
deployment sets nothing here. They are for a deployment that runs the image some other way.
...
`compose.prod.yml` is a second, independent path for deployments without Traefik: nginx terminates TLS itself from `deploy/certs/tls.crt` and `deploy/certs/tls.key`, and images are built locally rather than pulled. That nginx runs as an unprivileged user, listening on 8080 and 8443 with Compose publishing 80 and 443 in front of them, so **the key must be readable by uid 101** — `chown 101 deploy/certs/tls.key && chmod 640 deploy/certs/tls.key`. A key only root can read makes the container exit at startup with a permission error naming the file. Secrets are read from `./.secrets/{postgres_password,openbao_token,admin_password}`. Both files must be provided before it will start; neither is in source control. The GitLab pipeline does not use this file.
...
Eight mechanisms are deliberately per-instance and must move to a shared store before running more than one replica: the verified-key cache, the administrator password cache, the authorisation cache, the authentication throttle, the response store, the policy rate-limit buckets, the per-client rate-limit buckets, and the register of open console sessions that a password change ends.

The property that makes them safe on one instance is that every administrative change drops what it invalidated as part of the same request: rotating a secret, withdrawing a grant, disabling an API or purging the cache all take effect on the next call rather than at the end of a timeout. **That guarantee is what a second replica breaks.** An administrative call is received by one instance and invalidates only that one; the others go on serving what they hold until their own entry expires — within ten seconds for an authorisation, five minutes for a verified key, and for as long as its freshness lasts for a stored response. Revocation is the operation that suffers: an API key rotated on instance A is still accepted by instance B for up to five minutes. Add a shared store, or a bus these caches listen on, before running more than one.

Two more consequences of the same shape. A response store warms per replica, which costs latency and nothing else. A rate limit, on the other hand, is effectively multiplied by the number of replicas: divide the configured allowances accordingly, or move the buckets to Redis, before scaling out.
